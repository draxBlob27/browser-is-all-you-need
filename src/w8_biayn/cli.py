...
def data_aider_sft_plan(
    config: Path = typer.Option(
        Path("configs/aider_sft/pilot-v1.toml"),
        "--config",
        help="Strict Aider SFT profile TOML.",
    ),
) -> None:
...
def data_aider_sft_prepare_seccomp(
    ...
    """Download and validate the repository-pinned Moby seccomp profile."""
...
def data_aider_sft_inventory_promote(
    ...
    config: Path = typer.Option(
        Path("configs/aider_sft/pilot-v1.toml"),
        "--config",
        help="Profile TOML that fixes the approved manifest destination.",
    ),
) -> None:
...
def data_aider_sft_build(
    ...
    acknowledge_paid_llm_calls: bool = typer.Option(
        ...
        help="Permit budgeted authoring calls after all approval gates; source-only profiles reject it.",
    ),
) -> None:
...
def data_aider_sft_finalize(
    ...
    """Freeze an approved split, render/screen rows, and write readiness."""
...
def data_aider_sft_verify(
    ...
    """Recompute readiness and evidence without network access."""
...
def data_aider_sft_export(
    ...
    """Export the allowlist-only internal SLIME consumer bundle."""
...
def data_aider_sft_verify_export(
    ...
    """Independently reconcile the sanitized consumer bundle."""
