...
from w8_biayn.integrations import moonlight_fixed26_account_analogs_aider_tasks as tasks
...
def _patch_paths(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    ...
    out = expansion / "aider-fixed26-analogs/fixed26-b003-bank-account"
...
def test_spec_and_owner_have_exact_50_unique_ordered_roots() -> None:
    ...
    assert rows[0].task_id == "f26acc-library-card-desk"
    assert rows[-1].task_id == "f26acc-food-truck-shift"
    spec_ids = re.findall(r"^\| `(f26acc-[a-z0-9-]+)` \|", tasks.CURRICULUM.read_text(), re.M)
