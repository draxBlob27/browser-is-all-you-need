...
Current Aider task work is limited to generated local artifacts under
`.w8-biayn/data/aider-tasks/` and their clean-room source documents under
`docs/aider-synthetic/`. Read [the current scope](docs/AIDER_SFT_SCOPE.md)
before changing either surface. These are local task candidates, not SFT rows,
a dataset release, training authorization, or benchmark-uplift evidence.

The historical pipeline document and `w8-biayn data aider-sft ...` commands
remain repository surfaces only; they are not active requirements for current
local task authoring.
...
The remaining pipeline commands and profile notes in this historical section
are retained for reference only. They do not apply to the current local
...
For the user-authorized all-task local projection, match the established
`.w8-biayn/data/aider-tasks-sft/sft/train.jsonl` row shape and convert every
real task below the parallel reverify tree with the repo-owned CLI:
...
The converter excludes every `.state` path, including hard-rule adversarial
controls; preserves the reference's four top-level and three metadata fields;
deterministically qualifies only colliding leaf task IDs; and rejects private
path markers in user prompts. Its one audited balanced-tree owner-spec appendix
sanitation is recorded in the sibling manifest alongside source and output
hashes. This is a local SFT projection, not a finalized dataset release,
tokenizer/mask proof, or training authorization.
