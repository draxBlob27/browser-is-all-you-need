#!/usr/bin/env bash
# Fast 8x H100 Miles GRPO LoRA rank-16 runner for clean-room Aider C++.
...
export MILES_RUN_ROOT="${MILES_RUN_ROOT:-${REPO_ROOT}/.glm47-posttraining/miles/glm47-h100-aider-cpp/runs/${RUN_ID}}"
export MILES_AIDER_DATA_DIR="${MILES_AIDER_DATA_DIR:?Set MILES_AIDER_DATA_DIR to the verified private RL bundle}"
export MILES_AIDER_TASKS_DIR="${MILES_AIDER_TASKS_DIR:-${MILES_AIDER_DATA_DIR}/tasks}"
...
export GLM47_AIDER_REWARD_WORKERS="${GLM47_AIDER_REWARD_WORKERS:-32}"

export GLM47_EXPERIMENT_ID="${GLM47_EXPERIMENT_ID:-${RUN_ID}}"
export MILES_WANDB_PROJECT="${MILES_WANDB_PROJECT:-glm47-aider-cpp-posttraining}"
...
export WANDB_TAGS="${WANDB_TAGS:-canonical,aider-cpp,one-shot,grpo}"
...
exec "${REPO_ROOT}/scripts/train_aider_grpo.sh" "$@"
