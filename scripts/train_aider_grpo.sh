#!/usr/bin/env bash
# Miles GRPO LoRA rank-16 runner for the Aider clean-room C++ task.
...
RUN_ID="${MILES_RUN_ID:-glm47_h100_aider_cpp_lora_r16_$(date +%Y%m%d_%H%M%S)}"
STAGE_STARTED_AT="${SECONDS}"
RUN_ROOT="${MILES_RUN_ROOT:-${REPO_ROOT}/.glm47-posttraining/miles/glm47-h100-aider-cpp/runs/${RUN_ID}}"
DATA_DIR="${MILES_AIDER_DATA_DIR:?Set MILES_AIDER_DATA_DIR to a verified private Aider RL bundle}"
TASKS_DIR="${MILES_AIDER_TASKS_DIR:-${DATA_DIR}/tasks}"
if [ ! -f "${DATA_DIR}/manifest.json" ]; then
  echo "Missing Aider RL bundle manifest: ${DATA_DIR}/manifest.json" >&2
  exit 2
fi

HF_CHECKPOINT="${MILES_HF_CHECKPOINT:-/root/models/GLM-4.7-Flash}"
AIDER_TOKENIZER="${MILES_AIDER_TOKENIZER:-${HF_CHECKPOINT}}"
...
read -r ADMITTED_RESPONSE_BUDGET ADMITTED_MAX_PROMPT_TOKENS ADMITTED_MAX_TOTAL_TOKENS < <(
  "${PYTHON_BIN}" -c 'import json,sys; m=json.load(open(sys.argv[1], encoding="utf-8")); print(m["max_response_budget"], m["max_prompt_tokens"], m["max_total_tokens"])' "${DATA_DIR}/manifest.json"
)
...
ROLLOUT_MAX_RESPONSE_LEN="${MILES_ROLLOUT_MAX_RESPONSE_LEN:-${ADMITTED_RESPONSE_BUDGET}}"
...
EVAL_MAX_RESPONSE_LEN="${MILES_EVAL_MAX_RESPONSE_LEN:-${ADMITTED_RESPONSE_BUDGET}}"
...
WANDB_PROJECT="${MILES_WANDB_PROJECT:-glm47-aider-cpp-posttraining}"
WANDB_GROUP="${MILES_WANDB_GROUP:-glm47-h100-aider-cpp-lora-r16}"
...
echo "admitted_max_prompt_tokens=${ADMITTED_MAX_PROMPT_TOKENS}"
echo "admitted_max_total_tokens=${ADMITTED_MAX_TOTAL_TOKENS}"
...
# Aider reward is Docker-only and the bundle must pass all immutable checks.
if ! command -v docker >/dev/null 2>&1; then
  echo "Missing docker CLI required by the Aider grader." >&2
  exit 2
fi
if ! docker image inspect "${GLM47_AIDER_GRADER_IMAGE:-glm47-aider-cpp-grader:1}" >/dev/null 2>&1; then
  echo "Missing Aider C++ grader image: ${GLM47_AIDER_GRADER_IMAGE:-glm47-aider-cpp-grader:1}" >&2
  exit 2
fi
PYTHONPATH="${REPO_ROOT}/src:${PYTHONPATH:-}" "${PYTHON_BIN}" \
  -m glm47_posttraining.aider_rl.dataset ready --root "${DATA_DIR}" --tokenizer "${AIDER_TOKENIZER}"
if [ ! -s "${DATA_DIR}/grpo/train.jsonl" ]; then
  echo "Missing or empty Aider GRPO train data: ${DATA_DIR}/grpo/train.jsonl" >&2
  exit 2
fi
if [ "${N_SAMPLES_PER_PROMPT}" -lt 2 ]; then
  echo "Aider GRPO requires MILES_N_SAMPLES_PER_PROMPT >= 2 (use 8 initially)." >&2
  exit 2
fi
if [ "${SEQ_LENGTH}" -lt "${ADMITTED_MAX_TOTAL_TOKENS}" ] || \
   [ "${SEQ_LENGTH}" -lt "$((ADMITTED_MAX_PROMPT_TOKENS + ROLLOUT_MAX_RESPONSE_LEN))" ]; then
  echo "MILES_SEQ_LENGTH=${SEQ_LENGTH} cannot contain the admitted Aider prompt/response budget." >&2
  exit 2
fi
...
finalize_wandb() {
  ...
  PYTHONPATH="${REPO_ROOT}/src:${PYTHONPATH:-}" "${PYTHON_BIN}" \
    "${REPO_ROOT}/scripts/publish_aider_results.py" \
...
export MILES_AIDER_DATA_DIR="${DATA_DIR}"
export MILES_AIDER_TASKS_DIR="${TASKS_DIR}"
export GLM47_AIDER_GRADER_IMAGE="${GLM47_AIDER_GRADER_IMAGE:-glm47-aider-cpp-grader:1}"
export GLM47_AIDER_INFRASTRUCTURE_RETRIES="${GLM47_AIDER_INFRASTRUCTURE_RETRIES:-2}"
...
EVAL_ARGS=(
  ...
  --eval-prompt-data aider_cpp "${EVAL_PROMPT_DATA:-${DATA_DIR}/eval/validation.jsonl}"
...
RUNTIME_ENV_JSON="{
  \"env_vars\": {
    ...
    \"MILES_AIDER_DATA_DIR\": \"${DATA_DIR}\",
    \"MILES_AIDER_TASKS_DIR\": \"${TASKS_DIR}\",
    \"GLM47_AIDER_GRADER_IMAGE\": \"${GLM47_AIDER_GRADER_IMAGE}\",
    \"GLM47_AIDER_REWARD_WORKERS\": \"${GLM47_AIDER_REWARD_WORKERS:-8}\",
