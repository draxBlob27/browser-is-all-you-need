#!/usr/bin/env bash
# Moonlight-16B-A3B SLIME runner for Aider Polyglot C++ base evaluation.

set -euo pipefail

STAGE="${1:-${SLIME_POLYGLOT_STAGE:-}}"
if [ -z "${STAGE}" ]; then
...
PYTHON_BIN="${SLIME_POLYGLOT_PYTHON:-python3}"
...
ROLLOUT_BATCH_SIZE="${SLIME_POLYGLOT_ROLLOUT_BATCH_SIZE:-1}"
GLOBAL_BATCH_SIZE="${SLIME_POLYGLOT_GLOBAL_BATCH_SIZE:-1}"
...
POLYGLOT_SANDBOX_IMAGE="${W8_SLIME_POLYGLOT_SANDBOX_IMAGE:-w8-biayn-polyglot-cpp:latest}"
POLYGLOT_TEST_TIMEOUT_SECONDS="${W8_SLIME_POLYGLOT_TEST_TIMEOUT_SECONDS:-180}"
...
POLYGLOT_SOURCE="$(absolute_path "${POLYGLOT_SOURCE}")"
...
prepare_data() {
  if [ ! -d "${POLYGLOT_SOURCE}" ]; then
    echo "Missing Polyglot benchmark checkout: ${POLYGLOT_SOURCE}" >&2
    echo "Clone it first, for example:" >&2
    echo "  git clone https://github.com/Aider-AI/polyglot-benchmark .w8-biayn/data/polyglot-benchmark" >&2
    exit 2
  fi
  BUILD_DATA_ARGS=(
    -m w8_biayn.integrations.slime_polyglot_cpp build-data
    --source-root "${POLYGLOT_SOURCE}"
    --out "${DATA_DIR}"
    --profile "${SLIME_POLYGLOT_PROFILE:-moonlight-polyglot-cpp}"
    --run-id "${RUN_ID}"
    --force
  )
...
wandb_args() {
  ...
  if [ -n "${WANDB_KEY}" ] || [ "${WANDB_ALREADY_LOGGED_IN}" = "1" ] || [ -n "${SLIME_WANDB_PROJECT:-}" ]; then
    WANDB_ARGS=(
      --use-wandb
      --wandb-project "${SLIME_WANDB_PROJECT:-slime-moonlight-polyglot-cpp}"
...
stage_args() {
  ...
  ALGO_ARGS=()
  CUSTOM_ARGS=(--custom-rm-path w8_biayn.integrations.slime_polyglot_cpp.reward_func)
...
env = {
    ...
    "W8_SLIME_POLYGLOT_SANDBOX_IMAGE": os.environ.get("W8_SLIME_POLYGLOT_SANDBOX_IMAGE", "${POLYGLOT_SANDBOX_IMAGE}"),
    "W8_SLIME_POLYGLOT_TEST_TIMEOUT_SECONDS": os.environ.get("W8_SLIME_POLYGLOT_TEST_TIMEOUT_SECONDS", "${POLYGLOT_TEST_TIMEOUT_SECONDS}"),
    "W8_SLIME_POLYGLOT_INCLUDE_LOGS": os.environ.get("W8_SLIME_POLYGLOT_INCLUDE_LOGS", "0"),
...
wandb_project=${SLIME_WANDB_PROJECT:-slime-moonlight-polyglot-cpp}
...
submit_slime_job() {
  ...
  echo "Moonlight Polyglot C++ SLIME stage: ${STAGE}"
