...
# flex routes through DeepEP kernels, which are Hopper-only (SM90); alltoall
# works on every arch. Opt into flex explicitly on H100 hosts.
GLM_MOE_TOKEN_DISPATCHER_TYPE="${SLIME_GLM_MOE_TOKEN_DISPATCHER_TYPE:-alltoall}"
...
# Group size MUST be >= 2: one sample per prompt makes the GRPO advantage
# reward - mean(group) == 0 for every sample (zero learning signal, kl NaN).
GRPO_N_SAMPLES_PER_PROMPT="${SLIME_GRPO_N_SAMPLES_PER_PROMPT:-8}"
# Default: one train step per rollout (SLIME's own global-batch rule).
GRPO_GLOBAL_BATCH_SIZE="${SLIME_GRPO_GLOBAL_BATCH_SIZE:-$((GRPO_ROLLOUT_BATCH_SIZE * GRPO_N_SAMPLES_PER_PROMPT))}"
