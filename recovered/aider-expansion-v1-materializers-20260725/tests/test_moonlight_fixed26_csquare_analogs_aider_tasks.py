...
from w8_biayn.integrations import moonlight_fixed26_csquare_analogs_aider_tasks as tasks
...
def _patch_paths(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    ...
    out = expansion / "aider-fixed26-analogs/fixed26-b008-crypto-square"
...
def test_spec_and_owner_have_exact_50_unique_ordered_roots() -> None:
    ...
    assert rows[0].task_id == "f26csq-harbor-manifest-columns"
    assert rows[-1].task_id == "f26csq-ledger-column-swap"
    spec_ids = re.findall(r"^\| `(f26csq-[a-z0-9-]+)` \|", tasks.CURRICULUM.read_text(), re.M)
    ...
    assert sum(row.project_support for row in rows) == 10
...
def test_core_screen_has_1225_pairs_controls_and_lineage(
    ...
    assert (renamed / "f26csq-quay-manifest-columns.h").is_file()
    assert not (renamed / "f26csq-harbor-manifest-columns.h").exists()
    for name in tasks.CONTROL_NAMES:
        ...
        for changed in descriptor["changed_files"]:
            ...
            if changed.replace("f26csq-quay-manifest-columns", tasks.TASKS[0].task_id) != changed:
