...
from w8_biayn.integrations import moonlight_fixed26_radix_analogs_aider_tasks as tasks
...
def _patch_paths(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    ...
    out = expansion / "aider-fixed26-analogs/fixed26-b001-all-your-base"
...
def test_spec_and_owner_have_exact_50_unique_ordered_roots() -> None:
    ...
    assert rows[0].task_id == "f26rad-ledger-invalid-radix"
    assert rows[-1].task_id == "f26rad-canonical-output-matrix"
    spec_ids = re.findall(r"^\| `(f26rad-[a-z0-9-]+)` \|", tasks.CURRICULUM.read_text(), re.M)
