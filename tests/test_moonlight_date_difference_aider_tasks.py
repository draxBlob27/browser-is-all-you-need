...
def test_date_difference_curriculum_materializes_every_proposed_task(tmp_path: Path) -> None:
...
def test_date_difference_references_make_whole_file_sft_answers(tmp_path: Path) -> None:
    root = dates.build(tmp_path)[0]
...
def test_date_difference_wrapper_is_executable() -> None:
    ...
    assert "moonlight_date_difference_aider_tasks" in wrapper.read_text()
