...
completed a verified local 75-row release. Its generated bytes remain ignored
by Git; no 96-root pilot has been released. Start with the
no-spend/runtime-identity gates:
...
The plan revalidates the tokenizer/template and seccomp hashes and reads the
compiler version/hash from inside the exact pinned grader image, never from the
host compiler. It also requires the exact clean pinned SLIME checkout used for
canonical qwen loss-mask evidence. Finalization resolves that checkout without
requiring SLIME on `PYTHONPATH` and initializes the mask runtime before any
per-task final gate; run-level preflight failures do not reject candidates.

The distinct `aider-sft-source-only-75-v1` profile is the no-authoring lane:
75 source-backed train roots, zero validation/test roots, and zero LLM roots.
It has no provider or budget configuration, rejects paid-call acknowledgement,
and cannot backfill a failed source. It still requires all source Docker and
sanitizer gates, contamination review, exact final split/release approvals,
...
The Docker `fsize` limit converts locked MiB to bytes, and Catch v1 discovery
accepts only zero or the exact parsed positive count as its exit status. Resume
reuses source terminal records only under the current admission fingerprint,
so stale mechanical failures and historical run-level failures misclassified
as late task rejections rerun, while an empty pool returns a structured
shortfall. Shared Catch support is stored and hashed once. LLM tasks use the repo-owned
C++17
...
only hash-allowlisted shared support/scaffold boilerplate. The final ID scan
matches whole slugs rather than hyphen-delimited substrings, and late rejection
journals bind renderer/final-screen policy fingerprints for safe retry after a
policy correction. Candidate admission and dataset split/release use separate
state machines; genuine late final-row failures invalidate the split and
trigger backfill before exact release review. Mutable
