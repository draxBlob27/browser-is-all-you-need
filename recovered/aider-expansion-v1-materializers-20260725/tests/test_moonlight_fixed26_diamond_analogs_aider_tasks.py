...
from w8_biayn.integrations import moonlight_fixed26_diamond_analogs_aider_tasks as tasks
...
def _patch_paths(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    ...
    out = expansion / "aider-fixed26-analogs/fixed26-b009-diamond"
...
def test_spec_and_owner_have_exact_50_unique_ordered_roots() -> None:
    ...
    assert rows[0].task_id == "f26dia-anvil-forge-hourglass"
    assert rows[-1].task_id == "f26dia-aviary-perch-diagonals"
    spec_ids = re.findall(r"^\| `(f26dia-[a-z0-9-]+)` \|", tasks.CURRICULUM.read_text(), re.M)
...
def test_core_screen_has_1225_pairs_controls_and_lineage(
    ...
    assert (renamed / "f26dia-smithy-forge-hourglass.h").is_file()
    assert not (renamed / "f26dia-anvil-forge-hourglass.h").exists()
    for name in tasks.CONTROL_NAMES:
        ...
        for changed in descriptor["changed_files"]:
            ...
            if changed.replace("f26dia-smithy-forge-hourglass", tasks.TASKS[0].task_id) != changed:
