...
def test_circular_deque_curriculum_materializes_every_proposed_task(tmp_path: Path) -> None:
...
def test_circular_deque_reference_files_make_a_whole_file_sft_answer(tmp_path: Path) -> None:
    root = deque.build(tmp_path)[0]
