...
from w8_biayn.integrations.moonlight_matrix_rotation_cases import (
    CASES,
    RotationCase,
)
...
DEFAULT_OUT = Path(
    ...
    "aider-text-grid-reshaping/matrix-rotation-90"
)
LEGACY_OUT = Path(
...
CURRICULUM = (
    ...
    "GLM47_FLASH_AIDER_POLYGLOT_CPP_MATRIX_ROTATION_90_CURRICULUM.md"
)
FAMILY_SPEC = (
    "docs/aider-tasks-spec/aider-text-grid-reshaping/matrix-rotation-90.md"
)
...
NORMALIZER = "matrix-rotation-90-hard-rule-v4-artifact-dimension-shingles"
PAIR_THRESHOLD = 0.84
FAMILY_ROOT_MIN = 20
FAMILY_ROOT_MAX = 20
...
def _generator_revision() -> str:
    ...
    for path in (
        Path(__file__),
        Path(__file__).with_name("moonlight_matrix_rotation_cases.py"),
    ):
...
def _header(case: RotationCase) -> str:
    return (
        "#pragma once\n"
        "#include <array>\n#include <cstddef>\n#include <cstdint>\n"
        "#include <stdexcept>\n#include <string>\n#include <vector>\n"
...
def _source(case: RotationCase) -> str:
    return (
...
def _negative(case: RotationCase) -> str:
...
def _test(case: RotationCase, body: str) -> str:
    return (
        f'#include "{case.task_id}.h"\n#include <string>\n#include <vector>\n'
...
def _cmake(case: RotationCase) -> str:
...
def _instructions(case: RotationCase) -> str:
...
Follow the declared mutation contract exactly. `rotate-aerial-tiles` mutates its
square grid only after complete validation; every other root is a pure value
transform. Validate dimensions, shapes, indices, symbols, duplicates, bounds,
and arithmetic before mutation or result construction. Invalid input returns
the default report with `valid == false` and no partial mutation. Valid empty
collections have only the identity behavior stated by the API examples.
...
Use the task-specific canonical ordering for diagnostics. Coordinate systems
and every orientation convention use the visible top-left or Cartesian origin
stated by the contract; do not infer hidden coordinate rules.

Required mechanism: {case.profile}. A renamed generic dense-grid rotation is
not an implementation of this contract. The task-specific discriminator is:
{case.negative_description}.
{"For this API, `quarter_turns` must be from 0 through 3 inclusive." if case.task_id == "rotate-aerial-tiles" else "The declarations and public example define the complete accepted input domain."}
"""


def _files(case: RotationCase) -> dict[str, str]:
    header = _header(case)
    config = {
        "authors": ["w8-biayn"],
    ...
    files = {
        ".docs/introduction.md": (
            f"# {case.title}\n\nA local clean-room rotation transformation task.\n"
        ),
...
def _remedy_markdown(case: RotationCase) -> str:
    disposition = "repair-in-place" if case.task_id == case.legacy_id else "replace"
    return f"""## Identity

Legacy task ID: `{case.legacy_id}`; replacement task ID: `{case.task_id}`; task-spec revision: 2; family ID: `matrix-rotation-90-v2/{case.task_id}`; disposition: `{disposition}`; source inventory: `matrix-rotation-legacy-v1`; license: repository-authored/pass; generator: `src/w8_biayn/integrations/moonlight_matrix_rotation_aider_tasks.py`; benchmark screen: pending. Selected prompt: `{PROMPT_PATH}` with `FAMILY_NAME=matrix-rotation-90` and `FAMILY_TYPE=aider-text-grid-reshaping`.
...
Valid input executes `{case.profile}` and returns `valid == true` with the documented complete state. Invalid dimensions, bounds, shapes, duplicate identities, symbols, turns, or encodings return the default invalid report. The repaired aerial root validates before mutating and otherwise leaves input unchanged; all replacement roots are pure. Empty input is accepted only where the public examples explicitly define an identity. Ordering and tie behavior is deterministic and task-specific. Arithmetic and indexing are checked before use. The visible test is the public boundary example and the private test covers invalid, empty, duplicate, and boundary behavior.

## Implementation invariant

Required mechanism: `{case.profile}`. Required reference marker: `{case.marker}`. Forbidden substitutes are the legacy allocate-and-copy square-grid template, domain or identifier renames, constants/policy-only copies, opposite-end variants, a generic dense rotation wrapper that replaces the named representation, hard-coded examples, and benchmark assets. The private negative fixture is: {case.negative_description}.
...
Compare normalized docs, APIs, references, visible tests, private tests, and
negative fixtures across all 20 v2 roots and all 26 bound official C++
holdouts using `{NORMALIZER}`. Every one of the 190 family pairs must pass
separately in all seven hard-rule dimensions: public API, owned state or
algorithm, mutation or selection rules, invalid and boundary behavior,
reference control flow, deterministic oracle, and topic-specific negative
fixture. The three coherent adversarial clones must be rejected by that same
seven-dimensional screen. All 520 benchmark pairs are mandatory; no near-match
waiver is allowed.
...
def _write_remedies(out: Path, force: bool) -> None:
    ...
    for case in CASES:
        ...
        record = {
            ...
            "family_id_before": "matrix-rotation-90-v1",
            "family_id_after": f"matrix-rotation-90-v2/{case.task_id}",
            "tree_hash_before": _tree_hash(LEGACY_OUT / case.legacy_id),
            "generator_path": "src/w8_biayn/integrations/moonlight_matrix_rotation_aider_tasks.py",
            ...
            "finding_ids": [
                ...
                "MATRIX-ROTATION-003-nondiscriminating-tests",
                "MATRIX-ROTATION-004-invalid-hard-rule-evidence",
            ],
            "disposition": (
            ...
            "user_inputs": {
                "FAMILY_NAME": "matrix-rotation-90",
        ...
        record.update(
            {
                ...
                "evidence_invalidation": (
                    "prior family screen used one aggregate overlap and a no-op "
                    "rename control; it did not prove every emitted pair differed "
                    "in all seven hard-rule dimensions"
                ),
            }
        )
        _write(
...
def build(out: Path = DEFAULT_OUT, force: bool = False) -> tuple[Path, ...]:
    ...
    if force or not (out / ".state/remedy").is_dir():
...
def _domain_nouns(case: RotationCase) -> set[str]:
...
def _artifact_features(root: Path, case: RotationCase | None = None) -> set[str]:
...
def _pair_result(
    left: Path,
    right: Path,
    left_case: RotationCase | None = None,
    right_case: RotationCase | None = None,
) -> dict[str, object]:
    score = _overlap(
...
def _selected_cpp_shingles(text: str, patterns: tuple[str, ...]) -> set[str]:
    selected = "\n".join(
        line for line in text.splitlines() if any(re.search(pattern, line) for pattern in patterns)
    )
    return _cpp_shingles(selected)


def _hard_rule_features(root: Path, case: RotationCase) -> dict[str, set[str]]:
    ...
    features = {
        # Longer syntax shingles distinguish materially different declaration
        # shapes while still erasing every user-defined identifier.
        ...
        # This dimension is separate, so a deliberately wrong implementation cannot
        # lower an aggregate score and manufacture diversity in the other six.
...
def _hard_rule_pair_result(
    left: Path,
    right: Path,
    left_case: RotationCase,
    right_case: RotationCase,
) -> dict[str, object]:
...
def _adversarial_relative_paths(root: Path) -> tuple[str, ...]:
    return (
        ...
        next(
            ...
            if path.name != "task_visible_test.cpp"
        ).name,
        ".meta/example.cpp",
        ".meta/negative.cpp",
...
def _make_adversarial_clone(
    ...
    if case.task_id != "rotate-aerial-tiles":
        _fail("adversarial_control_invalid", "controls require rotate-aerial-tiles")
    ...
    if variant == "domain-identifier-renamed":
        replacements = (
            (case.function, "rotate_ledger_layers"),
            ("Aerial", "Ledger"),
            ("aerial", "ledger"),
            ("Survey", "Ledger"),
            ("survey", "ledger"),
        ...
        replacements = (
            ("quarter_turns > 3U", "quarter_turns > 7U"),
            ("turn_survey_layers(g,4)", "turn_survey_layers(g,8)"),
            ("0 through 3 inclusive", "0 through 7 inclusive"),
        )
        summary = "coherent maximum-turn policy change from three to seven"
    else:
        replacements = (
            ("r.north_sum!=12", "r.south_sum!=18"),
            ("north edge", "south edge"),
            ("north-edge", "south-edge"),
            ("north_sum", "south_sum"),
            ("tiles.front()", "tiles.back()"),
        )
        summary = "coherent north-to-south endpoint selection change"
    changed = _apply_replacements(clone, paths, replacements)
    if variant == "domain-identifier-renamed":
        for suffix in (".h", ".cpp"):
            old_name = f"{case.task_id}{suffix}"
            new_name = "rotate-ledger-records" + suffix
            (clone / old_name).rename(clone / new_name)
            changed.append(f"{old_name} -> {new_name}")
    minimum = {
        "domain-identifier-renamed": 6,
        "constants-or-policy-only": 4,
        "opposite-end-selection": 5,
    }[variant]
    if len(changed) < minimum:
        _fail(
            "adversarial_control_invalid",
            f"{variant}: only changed {changed}",
        )
    combined = "\n".join(
    ...
    if variant == "domain-identifier-renamed":
        if case.function in combined or "rotate_ledger_layers" not in combined:
            _fail("adversarial_control_invalid", f"incomplete rename: {changed}")
    elif variant == "constants-or-policy-only":
        required = ("quarter_turns > 7U", "turn_survey_layers(g,8)", "0 through 7 inclusive")
        if any(value not in combined for value in required):
            _fail("adversarial_control_invalid", f"incomplete policy clone: {changed}")
    elif not all(value in combined for value in ("south_sum", "tiles.back()", "south edge")):
...
def _adversarial_clone_results(
    root: Path, case: RotationCase
) -> dict[str, dict[str, object]]:
    results: dict[str, dict[str, object]] = {}
    with tempfile.TemporaryDirectory(prefix="matrix-rotation-clones-") as tmp:
...
def _whole_format_failure(case: RotationCase, response: str) -> str | None:
...
def _prompt_contract_failure(case: RotationCase, instructions: str) -> str | None:
        ...
        record.update(
            {
                ...
                "changed_owner_paths": [
                    ...
                    "src/w8_biayn/integrations/moonlight_matrix_rotation_aider_tasks.py",
                    "src/w8_biayn/integrations/moonlight_matrix_rotation_cases.py",
                    "tests/test_moonlight_matrix_rotation_aider_tasks.py",
                    "examples/slime/moonlight_cpp_perf/prepare_matrix_rotation_aider_tasks.sh",
                ],
            }
        )
...
def verify_core(out: Path, *, repo_root: Path | None = None) -> dict[str, dict[str, object]]:
    ...
    archive_case = next(case for case in CASES if case.task_id == "rotate-aerial-tiles")
    ...
    screen = {
        "schema_version": "matrix-rotation-90-family-screen-v4",
...
def verify(out: Path) -> dict[str, dict[str, int]]:
    ...
    if missing:
        receipt = {
            "schema_version": "matrix-rotation-90-host-oracle-v2",
    ...
    for case in CASES:
        root = out / case.task_id
        with tempfile.TemporaryDirectory(prefix=f"matrix-rotation-{case.task_id}-") as tmp:
...
def _accept_docker_receipt(out: Path, receipt: dict[str, object]) -> dict[str, object]:
    if (
        receipt.get("schema_version") != "matrix-rotation-90-docker-sanity-v4"
    ...
    archive_case = next(
        case for case in CASES if case.task_id == "rotate-aerial-tiles"
    )
    with tempfile.TemporaryDirectory(prefix="matrix-rotation-receipt-controls-") as tmp:
        for variant in ADVERSARIAL_CONTROLS:
...
def docker_sanity(out: Path, *, result_out: Path | None = None) -> dict[str, object]:
    verify_core(out)
    with tempfile.TemporaryDirectory(prefix="matrix-rotation-docker-") as tmp:
...
print("sha256:"+d.hexdigest())' "$root")
  ...
  for mode in normal asan_ubsan; do
    ...
    receipt: dict[str, object] = {
        "schema_version": "matrix-rotation-90-docker-sanity-v4",
        "status": "pass",
        "evidence_class": "docker_sanity",
        "locked_oracle": False,
        "network": "none",
        ...
        "hard_rule_controls": {
            variant: {
                **control_counts[variant],
                ...
                "classification": (
                    "coherent clone rejected by seven-dimensional screen"
                ),
            }
            for variant in ADVERSARIAL_CONTROLS
        },
