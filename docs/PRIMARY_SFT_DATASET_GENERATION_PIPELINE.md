...
Status: **implementation available; source-only 75/0/0 verified locally; pilot not frozen**
Contract version: `aider-sft-pipeline-v1.3`
Default dataset profile: `aider-sft-pilot-v1`
Source-only dataset profile: `aider-sft-source-only-75-v1`

Contract V1.3 supersedes V1.2 and V1.1 before the 96-root pilot profile is
frozen into a ready dataset. The implementation now exists, but the pilot
profile name remains `aider-sft-pilot-v1` because no 96-root pilot lock or
bundle has been released. The distinct source-only profile has completed a
verified local 75-row release; its generated bytes and receipts remain ignored
by Git. After the first pilot lock or bundle, any semantic change to its schema,
grader, limits, rendering, tokenizer, or review policy requires a new profile
ID rather than an in-place reinterpretation.
...
The repo-owned implementation lives in `src/w8_biayn/aider_sft/` and is
exposed through `w8-biayn data aider-sft ...`. It includes no-spend
inventory/promotion, canonical source and LLM-assisted task construction,
image-bound oracle receipts, role-aware contamination, reviewed split/finalize
state, final-answer-only rendering, token/mask evidence, immutable readiness,
sanitized export, producer verification, and consumer verification.

The checked-in 96-root pilot config remains draft. The distinct source-only
config records one operator-bound frozen environment and a promoted,
fingerprint-approved 75-root inventory; every `plan` still revalidates those
local identities. It still requires real image-bound admission and all later
scope-specific human decisions before a ready dataset can exist. A frozen plan
is only a build preflight. One local source-only run completed those gates with
75 train rows and producer status `ready`; that does not claim a 96-root pilot,
training result, benchmark score, or uplift.
...
## Source-Only 75/0/0 Profile

`aider-sft-source-only-75-v1` is a separate, no-authoring profile for an
all-train dataset made only from the exact non-benchmark Exercism inventory in
Appendix A:

| Provenance | Train | Validation | Internal test | Total |
|---|---:|---:|---:|---:|
| Source-backed | **75** | **0** | **0** | **75** |
| LLM-assisted | **0** | **0** | **0** | **0** |

This profile does not reinterpret or satisfy the 96-root pilot. It has no
`[llm]` configuration, no usage-terms or LLM-task approval scopes, no
authoring calls, and no LLM backfill. Passing
`--acknowledge-paid-llm-calls` is an error. All 75 approved source roots are
assigned to train; if any source is rejected, deferred, or fails admission,
the run is incomplete instead of filling the shortfall with generated work.

The reduced provenance and split surface does not weaken task quality or
release evidence. The profile still requires the exact pinned inventory and
benchmark denylist, image-bound C++17 normal and sanitizer oracles, hidden
tests and references, contamination/family checks, final-answer-only rendering,
token and qwen-mask evidence, producer verification, sanitized export
verification, and fingerprint-bound approvals for `source_inventory`, every
`contamination_flag`, `final_split`, and `dataset_release`. Its ready
bundle contains 75 rows in `sft/train.jsonl` and empty validation/test
...
The repo-owned upstream registry exposes the exact keys `exercism-cpp`,
`aider`, and `aider-polyglot`. A clean clone can acquire the pinned commits with:
...
Those implemented commands verify the requested commit and clean-tree identity
and never fall back to a branch tip. Existing one-off checkouts are study
material, not pipeline inputs.
...
| Compiler identity | measured inside the exact pinned grader image; the host compiler is not accepted as evidence |
| LLM task scaffold | `aider-sft-cmake-catch-v1` |
| CMake generator | `Unix Makefiles` |
| Readiness receipt schema | `2` |
| GLM sequence length | `4096` |
| Tokenizer loader | `transformers==4.57.6`, with exact load kwargs `fix_mistral_regex=true` |
...
### Profile Freeze Gates

The implementation is present, but `build` must refuse paid calls or admission
until all of the following checked-in profile inputs exist and reconcile with
this document:
...
The source-only profile uses
`configs/aider_sft/source-only-75-v1.toml` and
`manifests/aider_sft/source-only-75-v1-source.json`. It requires the same
non-LLM identities above but deliberately omits provider, budget, authoring,
usage-terms, and LLM-semantic-review fields.
...
choosing values for the operator. The config schema rejects unknown
keys and binds every default into `config.lock.json`.
...
These commands are implemented through the repository CLI. They are the only
supported operator surface; do not replace them with a one-off data-munging
script. Use the `aider-sft` dependency extra for commands that load or verify
the pinned tokenizer:

```bash
# Materialize the exact SLIME source used for canonical loss-mask evidence.
uv run --extra aider-sft w8-biayn upstreams clone slime

# Materialize only the pinned tokenizer files; no model weights are fetched.
uv run --extra aider-sft w8-biayn data aider-sft prepare-tokenizer \
  --out .w8-biayn/models/glm-4.7-flash-tokenizer-7dd20894

# Materialize and hash-check the pinned Moby seccomp profile.
...
uv run --extra aider-sft w8-biayn data aider-sft inventory-promote \
...
uv run --extra aider-sft w8-biayn data aider-sft build \
  --config configs/aider_sft/pilot-v1.toml \
  --out .w8-biayn/data/aider-sft-pilot-v1 \
  --resume \
...
uv run --extra aider-sft w8-biayn data aider-sft review-export \
  ...
  --out /path/to/fresh-empty-review-export
...
- `prepare-tokenizer` downloads only the four pinned tokenizer/config files,
  refuses model weight shards, validates the exact repository/revision marker
  and chat-template hash, and records the exact loader kwargs;
- `prepare-seccomp` downloads only the pinned Moby `seccomp/v0.2.1` default
  profile and rejects any byte/hash or schema mismatch;
- `plan` and `finalize` require the exact clean pinned SLIME checkout. Token
  evidence loads `slime/utils/mask_utils.py` from `SLIME_ROOT` or the repo-owned
  `.cache/upstreams/slime` checkout without requiring the full SLIME package on
  `PYTHONPATH`;
- `plan` must never call an LLM API, start a model server, or mutate paid
  external state. It measures the compiler path, full version, and binary hash
  inside the locked grader image rather than substituting the host compiler;
...
- `review-export` is deterministic, requires a new or empty destination, and
  contains no hidden file contents in prompt-preview fields. It can export
  `llm_usage_terms` before paid calls and `dataset_release` only after final
  rendering/screening;
...
- `finalize` initializes the pinned tokenizer and SLIME mask generator once,
  before entering any per-task final gate. A run-level `profile_not_frozen` or
  `consumer_*` preflight failure propagates without changing candidate state or
  deleting the frozen split;
...
- all commands print paths and safe counts, never credentials or hidden task
  contents;
- every lane wrapper remains a thin wrapper around these commands.
...
For `aider-sft-source-only-75-v1`, the loop omits steps 2 through 4's LLM
work. Promote the exact 75-root source inventory, run `build --resume`
without the paid acknowledgement, resolve only contamination flags if any,
approve the exact 75/0/0 `final_split`, and complete the same
`finalize -> dataset_release -> finalize -> verify` sequence. A source
shortfall terminates incomplete because this profile has no generation path.
...
The implementation is a focused package rather than an expansion of the
one-row module into one large file. Its layout is:

```text
src/w8_biayn/aider_sft/
  __init__.py
  assets/aider-sft-cmake-catch-v1/CMakeLists.txt.in
  benchmark.py
  config.py
  contamination.py
  errors.py
...
The commands are wired through `src/w8_biayn/cli.py`. Keep reusing or
extracting proven helpers from `moonlight_aider_task_*` and
`slime_polyglot_cpp.py`; do not fork them into incompatible copies.
...
as a compile-only phase. Pinned Catch v1 may return the exact positive
discovered test count as its process status; discovery accepts only status zero
or a status equal to the parsed positive count. Any other nonzero status is a
failure.

The Docker `fsize` ulimit is expressed in bytes. The adapter must convert the
locked `file_size_mib` value with `MiB * 1024 * 1024`; treating MiB as KiB
can kill GCC while it writes an object file and must be regression-tested.
...
1. exact task ID/slug/path denylist. Final rendered rows match IDs only as
   whole slugs: adjacent ASCII letters, digits, underscores, or hyphens make a
   compound identifier rather than an exact denylist mention;
...
Format-contract and preservation prose must stay semantically complete but
compact enough that every frozen source-only row fits the locked 4096-token
sequence without truncation.
...
The producer resolves `MultiTurnLossMaskGenerator` directly from the exact
clean pinned SLIME checkout and constructs it once as a run-level preflight.
The `aider-sft` dependency extra therefore pins Transformers and Jinja for
tokenizer/template execution but does not install the full SLIME training
environment as a Python package. Checkout or adapter identity failure is not
evidence against the first row being rendered; it must not be converted into a
task rejection.
...
Both profiles keep mechanical work automated but require semantic review where
deterministic execution cannot prove intent. LLM-specific items below apply
only to the pilot profile.
...
reviewer identity, UTC timestamp, and optional bounded comment. The pilot
requires `source_inventory`, `llm_usage_terms`, `llm_task_semantics`,
`contamination_flag`, `final_split`, and `dataset_release`. The source-only
profile requires only `source_inventory`, `contamination_flag`, `final_split`,
and `dataset_release`. Each scope fingerprints only the bytes and identities
...
- Treat an older `final_screen_invalidated_split` terminal record as stale when
  its recorded late reason is run-level rather than `rejected_content`; rebuild
  that source from preserved candidate evidence on `--resume`.
- Bind renderer and final-screen policy fingerprints into task-scoped late
  rejection records. A missing or changed policy fingerprint makes the record
  stale and permits one deterministic retry from preserved candidate evidence;
  an unchanged fingerprint preserves a genuine terminal rejection.
...
All `consumer_*` codes are runtime/consumer preflight failures, whether caught
during producer finalization or after export. They do not reject an admitted
candidate and do not mutate or revoke an otherwise valid ready dataset.
Shared-support,
...
The focused no-spend suite in `tests/test_aider_sft_pipeline.py` covers the
initial implementation and handoff regressions. It is not dataset-readiness
evidence by itself. The complete acceptance suite must retain or add, at
minimum:
...
- source admission resume must reuse terminal records only when their
  implementation/input fingerprint is current; a stale mechanical rejection
  or a historical late rejection caused by a run-level preflight failure is
  rerun, and an empty admitted pool returns structured
  `source_only_shortfall` rather than a filesystem traceback;
...
1. all CLI commands above—including `inventory-promote` and sanitized
