...
from w8_biayn.integrations import moonlight_fixed26_dnd_character_analogs_aider_tasks as tasks
...
def _patch_paths(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    ...
    out = expansion / "aider-fixed26-analogs/fixed26-b010-dnd-character"
...
def test_spec_and_owner_have_exact_20_unique_ordered_roots() -> None:
    rows = tasks.TASKS
    assert len(rows) == tasks.EXPECTED_ROOTS == 20
    assert len({row.task_id for row in rows}) == 20
    assert rows[0].task_id == "f26dnd-harbor-tide-gauge"
    assert rows[-1].task_id == "f26dnd-park-permit-register"
    spec_ids = re.findall(r"^\| `(f26dnd-[a-z0-9-]+)` \|", tasks.CURRICULUM.read_text(), re.M)
    ...
    assert sum(row.project_support for row in rows) == 4
...
def test_core_screen_has_190_pairs_controls_and_lineage(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    out = _patch_paths(monkeypatch, tmp_path)
    tasks.build(out, testing=True)
    receipt = tasks.verify_core(out)
    assert receipt["root_count"] == 20
    ...
    assert len(screen["decisions"]) == 190
    ...
    assert all(control["changed_files"] for control in screen["controls"])
    assert all(control["mutation_note"] == tasks.CONTROL_MUTATION_NOTE for control in screen["controls"])
    control_root = out / ".state/controls"
    renamed = control_root / "domain-identifier-renamed"
    assert (renamed / "f26dnd-harbor-tide-meter.h").is_file()
    assert not (renamed / "f26dnd-harbor-tide-gauge.h").exists()
    for name in tasks.CONTROL_NAMES:
        descriptor = json.loads((control_root / name / ".control.json").read_text())
        assert descriptor["control"] == name
        assert descriptor["source"] == tasks.TASKS[0].task_id
        assert descriptor["changed_files"]
        for changed in descriptor["changed_files"]:
            control_file = control_root / name / changed
            assert control_file.is_file()
            if changed.replace("f26dnd-harbor-tide-meter", tasks.TASKS[0].task_id) != changed:
                continue  # renamed-path entries are their own rename evidence
            source_path = out / tasks.TASKS[0].task_id / changed
            assert not source_path.is_file() or source_path.read_text() != control_file.read_text()
...
def test_audit_is_subject_bound_and_records_local_family_verified(
    ...
    docker = {
        ...
        "normal_test_count": 40,
        "sanitizer_test_count": 40,
        "negative_rejections": 20,
        "negative_normal_rejections": 20,
        "negative_sanitizer_rejections": 20,
    ...
    assert report["confirmed_counts"]["roots"] == 20
    assert report["confirmed_counts"]["per_root_docker_receipts"] == 20
