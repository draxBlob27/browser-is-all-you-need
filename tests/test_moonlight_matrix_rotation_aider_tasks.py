...
from w8_biayn.integrations import moonlight_matrix_rotation_aider_tasks as tasks
from w8_biayn.integrations.moonlight_matrix_rotation_cases import CASES
...
def test_genuine_adversarial_clones_are_coherent_and_rejected(
    ...
    case = next(case for case in CASES if case.task_id == "rotate-aerial-tiles")
    ...
    for variant in tasks.ADVERSARIAL_CONTROLS:
        ...
        if variant == "domain-identifier-renamed":
            ...
            assert "rotate_ledger_layers" in combined
        elif variant == "constants-or-policy-only":
            assert "quarter_turns > 7U" in combined
            assert "turn_survey_layers(g,8)" in combined
            assert "0 through 7 inclusive" in combined
        else:
            assert "south_sum" in combined
            assert "tiles.back()" in combined
            assert "south edge" in combined
