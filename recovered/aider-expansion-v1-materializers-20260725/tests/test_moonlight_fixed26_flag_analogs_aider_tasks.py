...
from w8_biayn.integrations import moonlight_fixed26_flag_analogs_aider_tasks as tasks
...
def _patch_paths(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    ...
    out = expansion / "aider-fixed26-analogs/fixed26-b002-allergies"
...
def test_spec_and_owner_have_exact_50_unique_ordered_roots() -> None:
    ...
    assert rows[0].task_id == "f26flg-marina-signal-check"
    assert rows[-1].task_id == "f26flg-festival-stage-genres"
    spec_ids = re.findall(r"^\| `(f26flg-[a-z0-9-]+)` \|", tasks.CURRICULUM.read_text(), re.M)
