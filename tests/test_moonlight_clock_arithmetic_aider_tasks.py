...
def test_clock_arithmetic_materializes_every_proposed_task(tmp_path: Path) -> None:
    roots=clock.build(tmp_path);assert len(roots)==10;assert {p.name for p in roots}=={s.task_id for s in clock.TASKS}
    for root in roots:
...
def test_clock_arithmetic_reference_makes_whole_file_sft_answer(tmp_path: Path) -> None:
    root=clock.build(tmp_path)[0];task=moonlight_aider_task_sft.load_task(root)
...
def test_clock_arithmetic_wrapper_is_executable() -> None:
    wrapper=Path("examples/slime/moonlight_cpp_perf/prepare_clock_arithmetic_aider_tasks.sh")
    assert wrapper.exists() and wrapper.stat().st_mode&0o111
    assert "moonlight_clock_arithmetic_aider_tasks" in wrapper.read_text()
