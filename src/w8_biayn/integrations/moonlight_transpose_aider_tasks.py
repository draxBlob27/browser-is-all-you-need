...
from w8_biayn.integrations.moonlight_transpose_cases import (
    CASES,
    TransposeCase,
)
...
DEFAULT_OUT = Path(
    ...
    "aider-text-grid-reshaping/transpose"
)
LEGACY_OUT = Path(
...
CURRICULUM = (
    ...
    "GLM47_FLASH_AIDER_POLYGLOT_CPP_TRANSPOSE_CURRICULUM.md"
)
FAMILY_SPEC = (
    "docs/aider-tasks-spec/aider-text-grid-reshaping/transpose.md"
)
...
NORMALIZER = "transpose-hard-rule-v4-artifact-dimension-shingles"
PAIR_THRESHOLD = 0.84
FAMILY_ROOT_MIN = 20
FAMILY_ROOT_MAX = 20
...
def _generator_revision() -> str:
    ...
    for path in (
        Path(__file__),
        Path(__file__).with_name("moonlight_transpose_cases.py"),
    ):
...
def _header(case: TransposeCase) -> str:
    return (
        "#pragma once\n"
        "#include <array>\n#include <cstddef>\n#include <cstdint>\n"
        "#include <stdexcept>\n#include <string>\n#include <vector>\n"
...
def _source(case: TransposeCase) -> str:
    return (
...
def _negative(case: TransposeCase) -> str:
...
def _test(case: TransposeCase, body: str) -> str:
    return (
        f'#include "{case.task_id}.h"\n#include <string>\n#include <vector>\n'
...
def _cmake(case: TransposeCase) -> str:
...
def _instructions(case: TransposeCase) -> str:
...
Follow the declared mutation contract exactly. `transpose-square-in-place`
mutates its square grid only after complete validation; every other root is a
pure value transform. Validate dimensions, shapes, indices, symbols, duplicates, bounds,
...
use the visible top-left, graph-endpoint, packed-index, or encoded-storage
convention stated by the contract; do not infer hidden coordinate rules.

Required mechanism: {case.profile}. A renamed generic row-to-column loop is
not an implementation of this contract. The task-specific discriminator is:
{case.negative_description}.
The declarations and public example define the complete accepted input domain.
"""


def _files(case: TransposeCase) -> dict[str, str]:
    header = _header(case)
    config = {
        "authors": ["w8-biayn"],
    ...
    files = {
        ".docs/introduction.md": (
            f"# {case.title}\n\nA local clean-room transpose transformation task.\n"
        ),
...
def _remedy_markdown(case: TransposeCase) -> str:
    disposition = "repair-in-place" if case.task_id == case.legacy_id else "replace"
    return f"""## Identity

Legacy task ID: `{case.legacy_id}`; replacement task ID: `{case.task_id}`; task-spec revision: 2; family ID: `transpose-v2/{case.task_id}`; disposition: `{disposition}`; source inventory: `transpose-legacy-v1`; license: repository-authored/pass; generator: `src/w8_biayn/integrations/moonlight_transpose_aider_tasks.py`; benchmark screen: pending. Selected prompt: `{PROMPT_PATH}` with `FAMILY_NAME=transpose` and `FAMILY_TYPE=aider-text-grid-reshaping`.
...
Valid input executes `{case.profile}` and returns the documented complete state. Invalid dimensions, bounds, shapes, duplicate identities, symbols, permutations, queries, or encodings return the default invalid report. The in-place square root validates before mutating and otherwise leaves input unchanged; all other roots are pure. Empty input is accepted only where the public examples explicitly define an identity. Ordering and tie behavior is deterministic and task-specific. Arithmetic and indexing are checked before use. The visible test is the public boundary example and the private test covers invalid, empty, duplicate, and boundary behavior.

## Implementation invariant

Required mechanism: `{case.profile}`. Required reference marker: `{case.marker}`. Forbidden substitutes are the legacy shared nested-loop ledger template, domain or identifier renames, constants/policy-only copies, opposite-end variants, a generic dense transpose wrapper that replaces the named representation or encoding, hard-coded examples, and benchmark assets. The private negative fixture is: {case.negative_description}.
...
def _write_remedies(out: Path, force: bool) -> None:
    ...
    for case in CASES:
        ...
        record = {
            ...
            "family_id_before": "transpose-v1",
            "family_id_after": f"transpose-v2/{case.task_id}",
            "tree_hash_before": _tree_hash(LEGACY_OUT / case.legacy_id),
            "generator_path": "src/w8_biayn/integrations/moonlight_transpose_aider_tasks.py",
            "generator_revision": _generator_revision(),
            "finding_ids": [
                "TRANSPOSE-001-shared-core-template",
                "TRANSPOSE-002-semantic-duplicate-family",
                "TRANSPOSE-003-nondiscriminating-tests",
                "TRANSPOSE-004-invalid-hard-rule-evidence",
            ],
            "disposition": (
...
def _domain_nouns(case: TransposeCase) -> set[str]:
...
def _artifact_features(root: Path, case: TransposeCase | None = None) -> set[str]:
...
def _pair_result(
    left: Path,
    right: Path,
    left_case: TransposeCase | None = None,
    right_case: TransposeCase | None = None,
) -> dict[str, object]:
    score = _overlap(
...
def _hard_rule_features(root: Path, case: TransposeCase) -> dict[str, set[str]]:
...
def _hard_rule_pair_result(
    left: Path,
    right: Path,
    left_case: TransposeCase,
    right_case: TransposeCase,
) -> dict[str, object]:
...
def _make_adversarial_clone(
    ...
    if case.task_id != "transpose-call-center":
        _fail("adversarial_control_invalid", "controls require transpose-call-center")
    ...
    if variant == "domain-identifier-renamed":
        replacements = (
            (case.task_id, "transpose-ledger-records"),
            (case.function, "transpose_ledger_records"),
            ("Call", "Ledger"),
            ("call", "ledger"),
        )
        ...
        replacements = (
            ("long long weight=1", "long long weight=2"),
            ("r.checksum!=86", "r.checksum!=107"),
        )
        summary = "coherent checksum initial-weight change from one to two"
    else:
        replacements = (
            ("out.cells[c][r]=rows[r][width-1U-c]", "out.cells[c][r]=rows[r][(c+1U)%width]"),
            ("out.cells[c][r]=rows[r][c]", "out.cells[width-1U-c][r]=rows[r][c]"),
            ("{{1,4},{2,5},{3,6}}", "{{3,6},{2,5},{1,4}}"),
            ("{{1,4}\n,{2,5}\n,{3,6}\n}", "{{3,6}\n,{2,5}\n,{1,4}\n}"),
            ("r.checksum!=86", "r.checksum!=70"),
            ("coordinate bijection", "reverse coordinate bijection"),
        )
        summary = "coherent increasing-to-decreasing source-column selection"
    changed = _apply_replacements(clone, paths, replacements)
    if variant == "domain-identifier-renamed":
        for suffix in (".h", ".cpp"):
            old_name = f"{case.task_id}{suffix}"
            new_name = "transpose-ledger-records" + suffix
            (clone / old_name).rename(clone / new_name)
            changed.append(f"{old_name} -> {new_name}")
    minimum = {
        "domain-identifier-renamed": 7,
    ...
    if variant == "domain-identifier-renamed":
        if case.function in combined or "transpose_ledger_records" not in combined:
            _fail("adversarial_control_invalid", f"incomplete rename: {changed}")
    elif variant == "constants-or-policy-only":
        required = ("long long weight=2", "r.checksum!=107")
        if any(value not in combined for value in required):
            _fail("adversarial_control_invalid", f"incomplete policy clone: {changed}")
    elif not all(value in combined for value in ("width-1U-c", "(c+1U)%width", "{3,6}", "{1,4}", "r.checksum!=70", "reverse coordinate bijection")):
...
def _adversarial_clone_results(
    root: Path, case: TransposeCase
) -> dict[str, dict[str, object]]:
    results: dict[str, dict[str, object]] = {}
    with tempfile.TemporaryDirectory(prefix="transpose-clones-") as tmp:
...
def _whole_format_failure(case: TransposeCase, response: str) -> str | None:
...
def _prompt_contract_failure(case: TransposeCase, instructions: str) -> str | None:
        ...
        record.update(
            {
                ...
                "changed_owner_paths": [
                    ...
                    "src/w8_biayn/integrations/moonlight_transpose_aider_tasks.py",
                    "src/w8_biayn/integrations/moonlight_transpose_cases.py",
                    "tests/test_moonlight_transpose_aider_tasks.py",
                    "examples/slime/moonlight_cpp_perf/prepare_transpose_aider_tasks.sh",
                ],
            }
        )
...
def verify_core(out: Path, *, repo_root: Path | None = None) -> dict[str, dict[str, object]]:
    ...
    screen = {
        "schema_version": "transpose-family-screen-v4",
...
def verify(out: Path) -> dict[str, dict[str, int]]:
    ...
    if missing:
        receipt = {
            "schema_version": "transpose-host-oracle-v2",
...
def _accept_docker_receipt(out: Path, receipt: dict[str, object]) -> dict[str, object]:
    if (
        receipt.get("schema_version") != "transpose-docker-sanity-v4"
    ...
    archive_case = next(
        case for case in CASES if case.task_id == "transpose-call-center"
    )
    with tempfile.TemporaryDirectory(prefix="transpose-receipt-controls-") as tmp:
        for variant in ADVERSARIAL_CONTROLS:
...
def docker_sanity(out: Path, *, result_out: Path | None = None) -> dict[str, object]:
    verify_core(out)
    with tempfile.TemporaryDirectory(prefix="transpose-docker-") as tmp:
...
print("sha256:"+d.hexdigest())' "$root")
  ...
  for mode in normal asan_ubsan; do
    ...
    receipt: dict[str, object] = {
        "schema_version": "transpose-docker-sanity-v4",
        "status": "pass",
        "evidence_class": "docker_sanity",
        "locked_oracle": False,
        "network": "none",
