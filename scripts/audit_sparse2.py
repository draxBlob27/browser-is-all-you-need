...
audit_6 = json.loads((state_dir / "audits/audit-06.json").read_text())
manifest = json.loads((state_dir / "materialization-manifest.json").read_text())
docker = json.loads((state_dir / "docker-sanity.json").read_text())

components = audit_6["subject_components"].copy()
components["docker_sanity_hash"] = sha((state_dir / "docker-sanity.json").read_bytes())
components["manifest_file_hash"] = sha((state_dir / "materialization-manifest.json").read_bytes())

components_str = json.dumps(components, sort_keys=True, separators=(',', ':'))
audit_subject_hash = sha(components_str)

audit_id = "audit-07"
candidate_roots = manifest["task_ids"]
...
audit_json = {
  ...
  "dispositions": {
    root: "train" for root in candidate_roots
  },
  "rejected_root_ids": [],
  "review_root_ids": [],
  "replaced_root_ids": [],
  "retained_root_ids": candidate_roots,
  "retained_count": 25
}
...
- `AUDIT-03-F004`: **closed for this exact subject**.
...
- `AUDIT-03-F006`: **closed for this exact subject**.
- `AUDIT-03-F007`: **closed for this exact subject**.
